...
#### End user side
create a `SocketConnection` to a server, and call the send method directly with a declared struct (from the `rnet_payloads` pool) as a param. That is it. 

#### Dev side
add a new `struct` to the *rnet_payloads/payloads* folder, and add to the `payloads` module the struct name to the enum and to the `match` pattern in `dispatcher`
...
`rnet_payloads` is an example and setup crate that is no dependency of `rnet`. It should be cloned and updated by users directly. Must be a bin dependency
...
rnet_payloads = { path = "../rnet_payloads" } // clone it and update it to handle more payloads
```

Note: `rnet_payload` own git clone link soon(ish), for now : `svn export https://github.com/gpoblon/rnet/rnet_payloads`